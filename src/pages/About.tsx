    ...
    <div className="overflow-x-hidden relative">
      ...
      <section className="py-16 bg-card/40 border-y border-border relative z-10">
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
          ...
          <ScrollReveal direction="none" staggerChildren={0.15} className="grid grid-cols-1 md:grid-cols-2 gap-6 md:col-span-2 w-full">
            {/* Mission Card */}
            <motion.div 
              variants={{
              ...
              className="spotlight-card p-8 rounded-[20px] flex flex-col justify-between relative group overflow-hidden"
            >
              ...
              {timelineSteps.map((step, idx) => (
                <motion.div 
                  ...
                  variants={{
                    ...
                    visible: { opacity: 1, x: 0, transition: { type: "spring", stiffness: 85, damping: 14 } }
                  }}
                  onMouseMove={handleMouseMove}
                  className="relative group spotlight-card p-6 rounded-[20px]"
                >
                  {/* Timeline node */}
                  <span className="absolute -left-[31px] md:-left-[51px] top-8 h-4 w-4 bg-background border-2 border-accent-light rounded-full shadow-glow-accent group-hover:scale-110 transition-transform duration-300 z-10" />
